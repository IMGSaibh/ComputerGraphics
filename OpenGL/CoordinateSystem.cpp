...
// ------------------------------
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        The output of vertex shader requires the coordinates in clip-space
        OpenGL performs than perspective Division on clip-space to transform 
        them to normalized device coordiantes (NDC) to map them on screen-space
        this is called viewport-transform

               1.)  V_clip = M_Projection * M_View * M_Model * M_Local
               2.)  Screenspace = viewport-transform(clipspace);
        ...
        ********************************************************************/
        ...
        // glm::perspective creates large frustum that defines visible space
        // anything outside frustum will not end up in the clip space
        ...
        // we can move this in a setMat4() shader method
